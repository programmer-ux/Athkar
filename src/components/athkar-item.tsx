...
export function AthkarItem({ athkar, isCompleted, onToggleComplete }: AthkarItemProps) {
  ...
  return (
    <Card
      className={cn(
        'mb-4 cursor-pointer transition-opacity duration-300 ease-in-out',
        !isVisible ? 'opacity-0' : 'opacity-100',
        isCompleted && 'border-accent bg-secondary' // Optional styling for completed (before hiding)
      )}
      onClick={handleTap}
    ...
    >
      <CardContent className="p-4">
        <p className="arabic text-lg leading-relaxed mb-2">{athkar.text}</p>
